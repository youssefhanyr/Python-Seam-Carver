...
class HSeamCarver2:
    "Forward energy image compression, shrinks the image height"
    ...
    def __init__(self, picture, ans, pic2=None, w=None, h=None, x1=None, y1=None, x2=None, y2=None):
        ...
        if isinstance(picture, str):
            ...
            self.width, self.height = self.picture.height, self.picture.width
            ...
            self.width, self.height = h, w                # just as a heads up, H carvers differ from V in only that they flip the image
        self.graphV = None                                # they oprate the same, i flip the image after finishing the work vertically
        self.ans = ans
        self.y1, self.x1, self.y2, self.x2 = x1, y1, x2, y2
        self._construct()
        self.sortV = topological_sort(self.graphV)

    def _construct(self):
        pixels = self.__construct_list()
        self.graphV = DiGraph(pixels + 2)                   # two extra nodes for source and sink nodes
        for vertex in range(self.graphV.v):
            ...
            if vertex + self.width >= self.width * self.height + 1:
                continue
            elif w == 0:                                                     # constructs the image graph as shown in the readme
    ...
    def __construct_list(self):
        ...
        for y in range(0, self.height):
            z = [stuff[i*self.height+y] for i in range(0, self.width)]
            self._rgb.append(z)
        return self.width*self.height
    ...
    def energy(self, x, y):
        ...
        this is the horizontal difference caused by the removal of the node
        ...
        if x == self.width - 1:
            rx, bx, gx = (self._rgb[y][x][i] - self._rgb[y][x - 1][i] for i in range(3))
        elif x == 0:
            rx, bx, gx = (self._rgb[y][x + 1][i] - self._rgb[y][x][i] for i in range(3))
        else:
    ...
    def findvseam(self):
        """Driver function that finds the shortest path and returns the list""" # didn't change the name because its literally the same
        total_energy = 0.0
        finder = AcyclicSPMODMarkII(self.graphV, 0, self, self.sortV)
    ...
    def removeverticalseam(self, path):
        ...
        for pixel in path:
            pixel -= 1
            x, y = pixel % (self.width + 1), pixel //(self.width + 1)               # calculates the x and y of the points delete
            ...
            elif y == 0:                                                # could have been wrote better, instead of repeating code
        ...
        if flag == 0:
            return temp, temp2, self.height, self.width, self.y1, self.x1, self.y2, self.x2
        elif flag == 1:
            return temp, temp2, self.height, self.width, self.y1-1, self.x1, self.y2-1, self.x2
