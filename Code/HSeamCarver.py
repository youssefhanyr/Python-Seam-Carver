...
class HSeamCarver:
    """Backwards horizontal seam carver, shrinks height of picture"""
    ...
    def _construct(self):
        ...
        for vertex in range(self.graphV.v):
            ...
            if vertex + self.width >= self.width * self.height + 1:
                ...
                self.graphV.add_edge(vertex, vertex + self.width + 1)               # constructs the image graph as shown in the readme
            elif w == self.width - 1:
    ...
    def energy(self, x, y):
        ...
        if self.x1 is not None and self.x2 is not None and self.y1 is not None and self.y2 is not None: # checks if there is a target
            if (self.x1 <= x <= self.x2) and (self.y1 <= y <= self.y2):                                 # region first and multiply it
                return self.ans*1500.0                                                          # by 1 to protect or by -1 to target
